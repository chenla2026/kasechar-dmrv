...
export function inferBiocharControlledStatus(
  batch: BatchForm,
  options?: DecisionSignals & { packageReady?: boolean },
): BiocharControlledStatus {
  if (options?.notAssessable) {
    return "NOT_ASSESSABLE";
  }

  if (options?.needsHumanReview || (options?.riskFlags?.length ?? 0) > 0) {
  ...
  if (options?.packageReady) {
    return "READY_FOR_AUDIT_PACKAGE";
  }
...
export function inferEudrControlledStatus(
  batch: BatchForm,
  options?: EudrDecisionSignals & { packageReady?: boolean },
): EudrControlledStatus {
  const checklist = buildEudrChecklist(batch);

  if (options?.notAssessable) {
    return "NOT_ASSESSABLE";
  }

  const geoValue = deriveGeoValue(batch);
  const coordSource = (batch.eudrCoordinateSource || "").trim().toLowerCase();
  const isTextOnlySource = [
    "text description", "text-only", "text", "informal", "unstructured", "unknown", ""
  ].includes(coordSource);
  const hasValidCoords = (hasMeaningfulText(batch.eudrLatitude) && hasMeaningfulText(batch.eudrLongitude)) ||
                         hasMeaningfulText(batch.eudrPolygonGeometry);

  if (options?.missingGeolocation || !hasMeaningfulText(geoValue) || (isTextOnlySource && !hasValidCoords)) {
    return "GEOLOCATION_MISSING";
  }

  if (options?.missingLegalityEvidence || !hasMeaningfulText(batch.eudrLegalityEvidence)) {
    return "LEGALITY_EVIDENCE_MISSING";
  }

  if (
    options?.deforestationRiskReviewRequired ||
    options?.missingDeforestationEvidence ||
    options?.missingLandUseHistoryEvidence ||
    options?.missingSatelliteEvidence ||
    !hasMeaningfulText(batch.eudrNoDeforestationEvidence) ||
    !hasMeaningfulText(batch.eudrLandUseHistoryEvidence) ||
    !hasMeaningfulText(batch.eudrSatelliteMapEvidence)
  ) {
    return "DEFORESTATION_RISK_REVIEW_REQUIRED";
  }
  ...
  if (checklist.missing.length > 0) {
    return "EUDR_EVIDENCE_INCOMPLETE";
  }

  return "EUDR_EVIDENCE_READY";
}
...
export function createAuditPackagePayload(
  batch: BatchForm,
  evidence: EvidenceRecord[],
  ...
  recs?: ReviewerRecommendation[]
): AuditPackage {
  const checklist = buildBiocharChecklist(batch);
  const riskFlags = signals.riskFlags ?? [];
  const packageId = `pkg-biochar-${Math.random().toString(36).slice(2, 10)}`;
  ...
  const basePayload = {
    ...
    evidenceDocuments: evidence.map((item) => ({
      id: item.id,
      title: item.title,
    ...
    extractedEvidence,
    evidenceGaps: checklist.missing.map((item) => item.label),
    auditNotes: [
      ...(riskFlags.length > 0 ? ["Risk flags: " + riskFlags.join(", ")] : []),
      signals.notAssessable ? "Model confidence indicates non-assessable condition" : "",
      signals.needsHumanReview ? "Human review required" : "",
    ]
      .filter(Boolean)
      .join(" ") || "No manual risk flags added yet.",
    controllerSummary: {
      riskFlags,
      confidence: signals.notAssessable ? 0.2 : 0.87,
      missingEvidence: checklist.missing.map((item) => item.label),
    },
    reviewerRecommendations: recs || generateReviewerRecommendations(batch, status, "EUDR_EVIDENCE_INCOMPLETE", signals, {}, evidence),
    provenanceSummary: {
      modelName: provenance?.modelName || "gemini-1.5-flash",
      promptVersion: provenance?.promptVersion || "kasechar-brain-v2",
      inputEvidenceIds: provenance?.inputEvidenceIds || evidence.map((e) => e.id),
      sourceReferences: provenance?.sourceReferences || evidence.map((e) => e.sourceAttribution || e.fileName || e.title),
      auditEventHashes: provenance?.auditEventHashes || [],
    },
  };

  const outputHash = computeDeterministicHash(JSON.stringify(basePayload));
  return {
    ...basePayload,
    outputHash,
  };
...
export function createEudrAuditPackagePayload(
  batch: BatchForm,
  evidence: EvidenceRecord[],
  extractedEvidence: GeminiExtraction,
  signals: EudrDecisionSignals,
  ...
  const checklist = buildEudrChecklist(batch);
  const riskFlags = signals.riskFlags ?? [];
  const packageId = `pkg-eudr-${Math.random().toString(36).slice(2, 10)}`;
  ...
  const basePayload = {
    ...
    project: {
      ...
      country: batch.country,
      region: batch.region,
      eudrProductionPlotGeolocation: deriveGeoValue(batch),
      eudrProductionPeriod: batch.eudrProductionPeriod,
      eudrCommodityBiomassSource: batch.eudrCommodityBiomassSource,
    },
    checklist: {
      completed: checklist.completed,
      total: checklist.total,
    ...
    eudrEvidence: {
      productionGeolocation: deriveGeoValue(batch),
      commodityOrBiomassSource: firstFilled([batch.eudrCommodityBiomassSource, batch.commodity]),
      producerOrSupplierIdentity: firstFilled([
        batch.eudrSupplierFarmerProducerIdentity,
        batch.supplierName,
        batch.producerIdentity,
      ]),
      productionPeriod: firstFilled([batch.eudrProductionPeriod, batch.productionDate]),
      noDeforestationEvidence: batch.eudrNoDeforestationEvidence,
      legalityEvidence: batch.eudrLegalityEvidence,
      landUseHistoryEvidence: batch.eudrLandUseHistoryEvidence,
      satelliteOrMapEvidence: batch.eudrSatelliteMapEvidence,
      riskStatus: batch.eudrRiskStatus,
      latitude: firstFilled([batch.eudrLatitude, batch.latitude]),
      longitude: firstFilled([batch.eudrLongitude, batch.longitude]),
      polygonGeometry: batch.eudrPolygonGeometry || "",
      crs: batch.eudrCrs || "WGS84",
      coordinateSource: batch.eudrCoordinateSource || "",
      captureDate: batch.eudrCaptureDate || "",
      linkedPlotRecord: firstFilled([batch.eudrLinkedPlotRecord, batch.plotPoint]),
    },
    missingEvidence: checklist.missing.map((item) => item.label),
    extractedEvidence: {
      eudrEvidence: {
        productionPlotGeolocation: batch.eudrProductionPlotGeolocation,
        productionPeriod: firstFilled([batch.eudrProductionPeriod, batch.productionDate]),
        commodityOrBiomassSource: batch.eudrCommodityBiomassSource,
        supplierFarmerProducerIdentity: firstFilled([
          batch.eudrSupplierFarmerProducerIdentity,
          batch.supplierName,
          batch.producerIdentity,
        ]),
        ...
        legalityEvidence: batch.eudrLegalityEvidence,
        landUseHistoryEvidence: batch.eudrLandUseHistoryEvidence,
        satelliteMapEvidence: batch.eudrSatelliteMapEvidence,
        riskStatus: batch.eudrRiskStatus,
        latitude: firstFilled([batch.eudrLatitude, batch.latitude]),
        longitude: firstFilled([batch.eudrLongitude, batch.longitude]),
        polygonGeometry: batch.eudrPolygonGeometry || "",
    ...
    evidenceGaps: [
      ...checklist.missing.map((item) => item.label),
      ...[
        signals.missingGeolocation ? "Production geolocation" : "",
        signals.missingLegalityEvidence ? "Legality evidence" : "",
        signals.missingDeforestationEvidence ? "Deforestation evidence" : "",
        signals.missingLandUseHistoryEvidence ? "Land-use history evidence" : "",
        signals.missingSatelliteEvidence ? "Satellite or map evidence" : "",
      ].filter(Boolean),
    ],
    controllerSummary: {
      riskFlags,
      confidence: signals.notAssessable ? 0.2 : 0.84,
    },
    reviewerRecommendations: recs || generateReviewerRecommendations(batch, "EVIDENCE_INCOMPLETE", status, {}, signals, evidence),
    provenanceSummary: {
      modelName: provenance?.modelName || "gemini-1.5-flash",
      promptVersion: provenance?.promptVersion || "kasechar-brain-v2",
      inputEvidenceIds: provenance?.inputEvidenceIds || evidence.map((e) => e.id),
